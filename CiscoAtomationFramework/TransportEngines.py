from CiscoAtomationFramework.Exceptions import AuthenticationException
...
class BaseEngine(ABC):
    ...
    def get_output(self, buffer_size=default_buffer, timeout=default_timeout, no_command_sent_previous=False):
        ...
        for x in range(self.commands_sent_since_last_output_get):
            ...
            start_with_hostname = last_line.startswith(self.hostname)
            has_prompt_ending = eny (x in last_line for x in standard_prompt_endings)
            while not (start_with_hostname and has_prompt_ending):
    ...
    def send_command_get_output(self, command, end=default_command_end, buffersze=default_buffer, timeout=default_timeout, delay=default_delay):
    ...
    def send_command_get_truncated_output(self, command):
        ...
        output = []
        while True:
            for line in self.get_output(timeout = 0.1):
                output.append(line)
    ...
    def _prompt_lookup(self, output_data):
        prompt = None
        for line in reversed(output_data.splitline()[-3:]):
